...
Import des données sur la population à l'échelle de la commune, et stratégie d'imputation pour les données manquantes
```{r}
...
data %>% ggplot(aes(x = densite)) +
  geom_histogram(bins=200, fill = 'cornflowerblue')

data$densite %>% arrange(densite)

filter(data, data$densite == 0)
```
