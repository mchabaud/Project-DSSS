...
Partie I : constitution de la base de données de panel à l'échelle de la TVS (sortie base_finale)
...
Import des bases des équipements à l'échelle de la commune, et stratégie d'imputation pour les données manquantes
```{r}
liste <- numeric(2012 - 2007 + 1)
for (i in (1:length(liste))){
  year <- 2006 + i 
  liste[i] <-paste0("bpe_",as.character(year), "/Csv/bpe", substr(as.character(year), nchar(as.character(year)) - 1,nchar(as.character(year))), "_ensemble.csv")
}

bpe_07_12 <- lapply(liste, function(i){read_csv2(i)[, c("DEPCOM", "TYPEQU", "NB_EQUIP", "AN")]})

liste <- numeric(2019 - 2012)
for (i in (1:length(liste))){
  year <- 2012 + i 
  liste[i] <-paste0("bpe_",as.character(year), "/Csv/bpe", substr(as.character(year), nchar(as.character(year)) - 1,nchar(as.character(year))), "_ensemble_com.csv")
}

bpe_13_19 <- lapply(liste, function(i){read_csv2(i)[, c("CODGEO","NB_D101", "NB_D106", "NB_D401")]})

#Ajout de la ligne AN pour les df 13-19
for (i in (1: length(liste))){
  bpe_13_19[[i]] <- bpe_13_19[[i]] %>% mutate(AN = 2012 + i)
}

#On filtre uniquement sur les catégories qui nous intéressent pour la liste 07_12
for (i in (1:(2012 - 2007 + 1))){
  bpe_07_12[[i]] <- bpe_07_12[[i]][bpe_07_12[[i]]$TYPEQU %chin% c("D101", "D106", "D401"),]
  bpe_07_12[[i]] <- bpe_07_12[[i]]%>%
  pivot_wider(names_from = TYPEQU, values_from = NB_EQUIP, values_fn = sum)

bpe_07_12[[i]] <- setnafill(bpe_07_12[[i]], cols = c("D101", "D106", "D401"), fill=0)
bpe_07_12[[i]] <-  bpe_07_12[[i]] %>% 
  rename(
    NB_D401 = D401,
    NB_D101 = D101, 
...
bpe <- bind_rows(bpe_07_12,bpe_13_19  )

A <- bpe %>% filter(!(CODGEO %chin% tvs$com)) %>% distinct(CODGEO)

tvs

bpe %<>% left_join(tvs, by = c("CODGEO" = "Code commune")) %>%
  group_by(`Code du territoire de vie-santé`, AN) %>%
  summarise(NB_D101 = sum(NB_D101), NB_D106 = sum(NB_D106), NB_D401 = sum(NB_D401))

bpe<-  bpe %>% rename(Code = `Code du territoire de vie-santé`)
bpe <- data.frame(bpe)
...
code <- '94048'
x <- ts(c(rep(NA,3), bpe$NB_D101[which(bpe$Code == code)]))
...
bpe <- bpe %>% arrange(Code, AN)
bpe <- bpe[!is.na(bpe$Code),]

#Boucle qui permet d'interpoler (en utilisant le filtre kalman)
for (code in unique(bpe$Code)){
  bpe$NB_D101[which(bpe$Code == code)] <- na_interpolation(ts(bpe$NB_D101[which(bpe$Code == code)] ))
   bpe$NB_D106[which(bpe$Code == code)] <- na_interpolation(ts(bpe$NB_D106[which(bpe$Code == code)] ))
   bpe$NB_D401[which(bpe$Code == code)] <- na_interpolation(ts(bpe$NB_D401[which(bpe$Code == code)]))
}
...
#Représentation de l'imputation effectuée (cas particulier Marolles-en-Brie)
code <- '94048'
x <- ts(c(pop_tvs_04_21[pop_tvs_04_21$Code == code,]$pop_totale[1],rep(NA, 5), pop_tvs_04_21[pop_tvs_04_21$Code == code,]$pop_totale[2:length(pop_tvs_04_21[pop_tvs_04_21$Code == code,]$pop_totale)], rep(NA, 3)))
imp <- na_kalman(x)
ggplot_na_imputations(x_with_na = x, imp)
...
for(code in unique(pop_tvs_04_21$Code)){
  pop_tvs_04_21 <- pop_tvs_04_21 %>% 
    tibble::add_row(
      Code   = rep(code, 9),
      pop_totale = rep(NA, 9), 
      annee = as.character(c((2000:2005), (2019:2021)))
    )
}
...
#Boucle qui permet d'interpoler (en utilisant le filtre kalman)
for (code in unique(pop_tvs_04_21$Code)){
  pop_tvs_04_21[pop_tvs_04_21$Code == code,]$pop_totale <- na_kalman(ts(pop_tvs_04_21[pop_tvs_04_21$Code == code,]$pop_totale), type = "level")
  #ajouter type = "level" permet d'éviter les messages d'erreur 
}



#A la fin, on ne garde que les données pour la période postérieure à 2004
pop_tvs_04_21 <- filter(pop_tvs_04_21, as.numeric(annee) > 2003)
```
...
deces_04_21_tvs <- deces_04_21 %>% filter(as.numeric(annee_deces) >= 2004) %>%
...
base_finale <- filter(base_finale, as.numeric(annee_deces) <= 2019)
